...
function exercise() {
	...
	$exercises = sqlQuery('select ex_id, ex_name from exercise order by ex_name');
	// print_r($subjects);
...
function exercisePlan($hour, $row)
{
    ...
    if ($rs === null) {
        ...
        $rs = sqlQuery("select concat(ep.ep_row,'_',ep.ep_col) as pos, ep.ep_id, ep.ex_id, ex.ex_name, ep.userid as uid
						from exercise_plan ep, exercise ex
						where ep.ex_id = ex.ex_id", 0);
    }

    print '<tr>';
    print '<td class="redips-mark blank">' . $hour . '</td>';
