        ...
        <section class="site-section-small section-blog-post">
            ...
            <div class="container">
                ...
                <div class="form-inline" style="margin: 50px">
                    <label for="username">Dish: </label><BR>
                    <input class="form-control mr-sm-2" type="text" id="foodname" placeholder="Enter the dish you like"> <BR>
                    <button class="btn btn-success btn-yellow" type="submit" onclick="output()">Search</button>
                </div>
        </div>
    </div>
    <div class="container" style="min-height: 100px; overflow: hidden" >
        <table class="table table-bordered" id="id_food_table" style="overflow-y: scroll; background-color:white; max-width:800px"> </table>
    </div>
    <script>
        var data_table = "<thead> <tr> <th>Description</th> <th>Type</th><th>Vitamin</th><th>Fibre</th><th>Sugar</th><th>Fat</th><th>Carbonhydrate</th> </tr></thead><tbody>";
        function output(){
            var x;
        ...
        function addtable(foodlist){  
            for(var i=0; i< foodlist.length; i++){
                data_table = data_table+ "<tr><td style='color:blue; pointer: curser;' id='"+foodlist[i].foodid+"'onclick=''>"+foodlist[i].foodname+"</td><td>"+foodlist[i].group;
