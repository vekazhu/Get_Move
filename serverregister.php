...
if (empty(trim($password_1))) {
  ...
  array_push($errors,"Password must include atleast 1 number.");
}elseif( !preg_match("#[a-z]+#", $password_1 ) ) {
  array_push($errors,"Password must include atleast 1 character.");
}
...
if (count($errors) == 0) {   
    // prepare and bind
    $insertstmt = $db->prepare("INSERT INTO user(username,email,password) VALUES (?, ?, ?)");
    $insertstmt->bind_param("sss", $nm, $em, $pw);
